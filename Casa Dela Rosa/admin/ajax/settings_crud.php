<?php

  require('../inc/db_config.php');
  require('../inc/settings_crud.php');
  adminLogin();

  if (isset($_POST['get_general'])) {
    $q = "SELECT * FROM setting WHERE sr_no=?";
    $values = [1];
    $res = select($q, $values, "i");
    $data = mysqli_fetch_assoc($res);
    $json_data = json_encode($data);
    echo $json_data;
  
  }

    if (isset($_POST['upd_general'])) {

      $frm_data = filteration($_POST);

      $q = "UPDATE setting SET site_title = ?, site_about = ? WHERE sr_no = ?";
      $values = [$frm_data['site_title'], $frm_data['site_about'], 1];
      $res = update($q, $values, 'sss');
      echo $res;
  }

    if (isset($_POST['upd_shutdown'])) {

      // Set shutdown to the value sent from the client (no toggle)
      $new_value = intval($_POST['upd_shutdown']);

      $q = "UPDATE `setting` SET `shutdown` = ? WHERE `sr_no` = ?";
      $values = [$new_value, 1];
      $res = update($q, $values, 'ii');
      echo $res;
  }

  if (isset($_POST['get_contacts'])) {
      $q = "SELECT * FROM contacts_details WHERE sr_no=?";
      $values = [1];
      $res = select($q, $values, "i");
      $data = mysqli_fetch_assoc($res);
      $json_data = json_encode($data);
      echo $json_data;
  }

  if (isset($_POST['upd_contacts'])) {

    $frm_data = filteration($_POST);

    $q ="UPDATE `contacts_details` SET `address`=?,`gmap`=?,`pn1`=?,`email`=?,`fb`=?,`iframe`=? WHERE `sr_no`=?";
    $values = [
        $frm_data['address'],
        $frm_data['gmap'],
        $frm_data['pn1'],
        $frm_data['email'],
        $frm_data['fb'],      // <-- Add this line
        $frm_data['iframe'],
        1
    ];
    $res = update($q, $values, 'ssssssi');
    echo $res;
 }
  if(isset($_POST['add_member']))
  {
    $frm_data = filteration($_POST);

    $img_r = uploadImage($_FILES['picture'],ABOUT_FOLDER);

    if($img_r == 'inv_img'){
      echo $img_r;
    }
    else if($img_r == 'inv_size'){
      echo $img_r;
    }
    else if($img_r == 'upd_failed'){
      echo $img_r;
    }
    else{
      $q = "INSERT INTO `team_details`(`name`, `picture`) VALUES (?,?)";
      $values = [$frm_data['name'], $img_r];
      $res = insert($q,$values,'ss');
      echo $res;
    }
  } 
  
  if(isset($_POST['get_members']))
  {
    $res = selectAll('team_details');
    
    while($row = mysqli_fetch_assoc($res))
    {
      $path = ABOUT_IMG_PATH;
      echo <<<data
         <div class="col-md-2 mb-3">
               <div class="card bg-dark text-white">
                <img src="$path$row[picture]" class="card-img">
                <div class="card-img-overlay text-end">
                  <button type="button" class="btn btn-sm btn-danger shadow-none">
                    <i class="bi bi-trash"></i>Delete
                  </button>
                </div>
                <p class="card-text text-center px-3 py-2">$row[name]</p>
               </div>
              </div> 
      data;
    }
    
  }
  if (isset($_POST['login'])) {
    $frm_data = filteration($_POST);

    $q = "SELECT * FROM users WHERE email=? LIMIT 1";
    $values = [$frm_data['email']];
    $res = select($q, $values, "s");
    $data = mysqli_fetch_assoc($res);

    if ($data && password_verify($frm_data['password'], $data['password'])) {
        // Set session or return success
        $_SESSION['user_id'] = $data['id'];
        echo json_encode(['status' => 'success', 'user' => $data['name']]);
    } else {
        echo json_encode(['status' => 'error', 'msg' => 'Invalid credentials']);
    }
}
?>